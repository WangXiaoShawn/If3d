...
def parse_args():
    ...
    parser.add_argument('--aff_path', type=str, default='Dataset/affectnet_clean', help='AffectNet dataset path.')
    ...
    parser.add_argument('--model_path', default='FinalCheckPoints/affecnet8sota.pth', help='Path to the model checkpoint.')
...
def run_test():
    ...
    if args.num_class == 7:
        ...
        plot_confusion_matrix(matrix, classes=class8_names, normalize=True,
                              title='AffectNet Confusion Matrix (acc: %0.2f%%)' % (acc * 100))
        plt.savefig(os.path.join('FusionConfusionMatrix', "Orig" + "_acc" + str(acc) + ".png"))
        plt.close()
