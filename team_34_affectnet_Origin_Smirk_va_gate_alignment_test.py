...
def parse_args():
    ...
    parser.add_argument('--checkpoint', type=str, default=os.path.join(base_dir, 'FinalCheckPoints/GateFusion.pth'), help='Path to the model checkpoint.')
...
def evaluate():
    ...
    plot_confusion_matrix(matrix, classes=class8_names, normalize=True,
                          title='Gate_Fusion Confusion Matrix (acc: %0.2f%%)' % (accuracy * 100))

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(os.path.join('FusionConfusionMatrix', f"Gate_Fusion.png"))
    plt.close()
