...
class Flatten(Module):
    def forward(self, input):
        return input.view(input.size(0), -1) # view就是reshape的一种(batch_size, channels, height, width) -> (batch_size, channels*height*width)

def l2_norm(input,axis=1):# 进行 L2 归一化 每一行
...
class MixedFeatureNet(Module):
    '''
    这段代码定义了一个名为 MixedFeatureNet 的神经网络模型。
    这个模型通过多个卷积块、深度卷积块和混合深度卷积块来提取输入图像的特征，并将其嵌入到一个固定大小的向量空间中。
    '''
    ...
    def forward(self, x):
        out = self.conv1(x) # 输入是112x112x3的图像，输出是56x56x64的特征图
