...
class DDAMNet(nn.Module):
    def __init__(self, num_class=7,num_head=2, pretrained=True):
        ...
        self.Linear = Linear_block(512, 512, groups=512, kernel=(7, 7), stride=(1, 1), padding=(0, 0)) #上面定义的
        self.flatten = Flatten()      
        self.fc = nn.Linear(512, num_class)
        self.bn = nn.BatchNorm1d(num_class)
        
    def forward(self, x):   # linear cobination
...
class CoordAtt(nn.Module):# 类实现了一种坐标注意力机制，通过同时考虑水平方向和垂直方向的信息来增强输入特征。让我们逐行解析这个类的定义和其前向传播方法：
    def __init__(self, inp, oup, groups=32): #groups：用于分组卷积的组数，默认为 32
        ...
        self.Linear_h = Linear_block(inp, inp, groups=inp, 
                                     kernel=(1, 7), stride=(1, 1), padding=(0, 0))#self.Linear_h：用于水平方向的卷积，卷积核大小为 (1, 7)。        
        self.Linear_w = Linear_block(inp, inp, groups=inp,
                                     kernel=(7, 1), stride=(1, 1), padding=(0, 0))#self.Linear_w：用于垂直方向的卷积，卷积核大小为 (7, 1)。
