...
# 导入您的模型
from networks.DDAM_smirkpic_fusion import DDAMNet_Smirk_Spatial_Alignment_SPPFusion
...
def parse_args():
    ...
    parser.add_argument('--checkpoint', type=str, default=os.path.join(base_dir, 'FinalCheckPoints/SPPFusion.pth'), help='Path to the model checkpoint.')
...
def evaluate():
    ...
    model = DDAMNet_Smirk_Spatial_Alignment_SPPFusion(num_class=args.num_class, num_head=args.num_head)
    ...
    plot_confusion_matrix(matrix, classes=class8_names, normalize=True,
                          title='SPP_Fusion Confusion Matrix (acc: %0.2f%%)' % (accuracy * 100))
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    plt.savefig(os.path.join('FusionConfusionMatrix', f"SPP_Fusion.png"))
    plt.close()
